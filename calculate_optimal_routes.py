...
comb = []
with open('ainutkertaiset_reitit.csv', 'r') as infile:
  newFileReader = csv.reader(infile) 
  for row in newFileReader:
    comb.append(row)
routes = [['start_station_id', 'end_station_id', 'distance', 'duration', 'geometry']]
...
for row in comb:
  coordinates = str(row[2]) + ',' + str(row[1]) + ';' + str(row[5]) + ',' + str(row[4])
  print(coordinates)
  ...
  route = Route(start_station_id=row[0], end_station_id=row[3], distance=distance, duration=duration, geom=geometry)
  session.add(route)
  counter = counter + 1
  if counter % 100 == 0:
