...
# Load districts from file and write them to table "district" in the database
...
for index, row in areas.iterrows():
  area = District(district_id=row['kokotun'], municipality=row['kunta'], major_district=row['suur'], base_district=row['perus'], sub_district=row['osa'], name=row['nimi'], geom=from_shape(row['geometry'], srid=3857))
  session.add(area)
for index, row in espoo.iterrows():
  espoo_area = District(district_id='0490000000', municipality='049', major_district="0", base_district='000', sub_district='000', name='Espoo', geom=from_shape(row['geometry'], srid=3857))
